...
def uiExample():
    ...
    st.text("Nifty 50 -  " + yf.Ticker("^NSEI").info['regularMarketPrice'])
    st.text("Sensex -  " + yf.Ticker("^BSESN").info['regularMarketPrice'])
    st.text("S&P 500 -  " + yf.Ticker("^GSPC").info['regularMarketPrice'])
    st.text("DOW Jones Industrial Average -  " + yf.Ticker("^DJI").info['regularMarketPrice'])
    st.text("Russel 2000 -  " + yf.Ticker("^RUT").info['regularMarketPrice'])
#     nsei =yf.Ticker('NSEI')
#     st.text(nsei.info)
